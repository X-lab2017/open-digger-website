---
slug: redis-analysis
title: Redis Modifies Open Source License! Are Cloud Providers Really Freeloading off the Open Source Community?
authors: [zsy, xiaoya]
tags: [redis]
---
...
This license change was primarily aimed at protecting Redis Inc.'s commercial interests, preventing cloud providers from using the open-source version to offer Redis SaaS services for a fee. Such actions are not uncommon; companies like Confluent, MongoDB, and Elastic have made similar license changes to their open-source projects to protect their rights. However, this move by Redis triggered anger among many developers, a significant reason being that the Redis community includes many external contributors, and such unilateral modifications of the license are seen as damaging to the community and harmful to these contributors.

So, who exactly is deeply involved in contributing to the Redis community?

## In-depth

The graph below shows the contribution distribution of the OpenRank [<sup>1</sup>](#refer-anchor) top ten developers in the Redis project community since 2020. We can tell that the Redis project community has been trending towards diversification. The contribution share of internal Redis developers has decreased from nearly 80% in 2020, and by the first quarter of 2024, the contribution share of the top ten developers within Redis had dropped to less than 40%. Many companies, including AWS, Alibaba Cloud, Tencent Cloud, and Ericsson, have been deeply involved in contributing to the Redis community for years, with their contribution intensity increasing annually.
...
<SimpleECharts option={{
  ...
  legend: {
    data: ['Redis', 'AWS', 'Aliyun', 'Tencent', 'Huawei', 'Ericsson', 'Netease', 'Mozilla', 'Meituan', 'Others']
  },
  backgroundColor: '#f5f5f5',
  grid: { left: '3%', right: '4%', bottom: '3%', containLabel: true },
  xAxis: [{ type: 'category', boundaryGap: false, data: [2020, 2021, 2022, 2023, 2024] }],
  yAxis: [{ type: 'value', min: 0, max: 100 }],
  series: [{"name":"Redis","type":"line","stack":"Total","areaStyle":{},"data":[77.24,63.36,61.1,47.06,36.75]},{"name":"AWS","type":"line","stack":"Total","areaStyle":{},"data":[7.11,10.92,13.76,18.32,15.47]},{"name":"Aliyun","type":"line","stack":"Total","areaStyle":{},"data":[2.48,0,0,4.31,11.5]},{"name":"Tencent","type":"line","stack":"Total","areaStyle":{},"data":[0,0,0,15.91,28.78]},{"name":"Huawei","type":"line","stack":"Total","areaStyle":{},"data":[5.98,3.33,3.83,5.35,0]},{"name":"Ericsson","type":"line","stack":"Total","areaStyle":{},"data":[0,4.42,5.2,6.03,7.49]},{"name":"Others","type":"line","stack":"Total","areaStyle":{},"data":[7.19,17.97,16.1,3.02,0]}]
}} />

At the end of June 2020, Salvatore Sanfilippo (@antirez), the original author of Redis, announced in a [blog post](http://antirez.com/news/133) that he was stepping down from daily maintenance work of the Redis community, handing over the community maintenance tasks to Yossi Gottlieb (@yossigo) and Oran Agra (@oranagra) from the then-called RedisLabs. At the same time, the aforementioned duo published an [article](https://redis.com/blog/new-governance-for-redis/) stating the initiation of a new community governance model, and together with Itamar Haber (@itamarhaber), formed the core development team of the Redis community. [The following month](https://redis.com/blog/redis-core-team-update/), Madelyn Olson (@madolson) from AWS and Zhao Zhao (@soloestoy) from Alibaba Cloud joined the core development team, which remained stable until the recent license change of Redis.

Besides the six core developers mentioned above, Zhu Binbin (@enjoy-binbin) from Tencent Cloud joined the Tencent Cloud database product department due to his long-term involvement in Redis. In addition to Zhao Zhao, Alibaba Cloud has three other developers who have appeared in the top ten contributors list over the years. Overall, the current cloud providers like AWS, Alibaba Cloud, Google, and Tencent Cloud have nearly 20 people actively contributing to the Redis community. The investment of cloud providers in the Redis community is evident and contrasts with the common perception of cloud providers freeloading off the open-source community.

## Split

It was precisely because of the involvement of numerous cloud provider contributors that, following Redis's announcement of the license change, Madelyn Olson from AWS immediately initiated a fork of Redis named [Valkey](https://github.com/valkey-io/valkey), with plans to host it [under the Linux Foundation](https://www.linuxfoundation.org/press/linux-foundation-launches-open-source-valkey-community). Google and Ericsson have already explicitly expressed their support for the development of the Valkey community.

Other cloud provider developers are almost left with no choice but to migrate to the Valkey project, as the new licensing terms for Redis exclude cloud providers, preventing them from continuing to contribute to the Redis community. It appears that Redis has no intention of allowing the community to deeply participate in further development. According to feedback from several Chinese committers, the permissions for the GitHub redis-committers team were revoked within a week, removing the external committers' repository write access and Issue/PR management access. Now, their permissions in the Redis project are essentially the same as those of ordinary users.

"In addition to participating in specific functional contributions to the Redis community, we also contribute back to the community the fixes and improvements in aspects such as functionality, performance, stability, and observability that we have accumulated in our cloud products. The rich user base of our cloud products also conveys a large number of real business needs to the upstream community. We believe it is our responsibility, and we trust that a thriving open-source community is worth our long-term maintenance," said Zhao Zhao of Alibaba Cloud.

From the data below, among the top ten contributors to Redis in 2024, aside from two developers from Redis Ltd., the remaining seven have already participated in the development of the Valkey project. This indicates that the Valkey project has effectively become the new community and is now in normal operation, while the developers at Redis Ltd. will continue to independently develop and maintain the Redis project.
